...
def WIT_Africa_drill(feat, crs, time_period, Output_dir, columnName, chunks=None):
    first_geom = feat['geometry']
    polyName = feat['properties'][columnName]
    print("\r", "working on polygon: " + polyName + ".  ",  end='')
    ...
    ls578_ds = deafrica_datahandling.load_masked_usgs(dc=dc,
                                                      products=[
                                                          'usgs_ls8c_level2_2'],
                                                      output_crs=crs,
                                                      min_gooddata=0.9,
                                                      ...
                                                      lazy_load=True,
                                                      align=(15, 15), dask_chunks=chunks,
                                                      resolution=(-30, 30),
                                                      **query)
...
#     ls578_ds.astype()
    ...
    print('wofls stuff')
    wofls = dc.load(product='ga_ls8c_wofs_2', like=ls578_ds,
                    fuse_func=wofs_fuser, dask_chunks=chunks)
    ...
    print('fractional cover stuff')
    # load fractional cover
    fc_ds = dc.load(product='ga_ls8c_fractional_cover_2', dask_chunks=chunks,
                    like=ls578_ds, measurements=['pv', 'npv', 'bs'])
    fc_ds = fc_ds.where(ls578_ds.red)

#     return fc_ds, ls578_ds, wet_wofs
...
def animated_timeseries_WIT(ds, df, output_path,
        ...
        if ((len(bands) == 3) | (len(bands) == 1)) & all([(b in ds.data_vars) for b in bands]):
            ...
            ax2.stackplot(df.index,
                          ...
                          labels=['open water',
                                  ...
                                  'dry veg',
                                  'bare soil',
                                  ], colors=pal, alpha=0.6, **pandasplot_kwargs)

            ax2.legend(loc='lower left', framealpha=0.6)

            df1 = pd.DataFrame({'wofs_area_percent': df.wofs_area_percent,
                               ...
                               'dry_veg_percent': df.wofs_area_percent+df.wet_percent+df.green_veg_percent+df.dry_veg_percent,
              'bare_soil_percent': df.dry_veg_percent+df.green_veg_percent+df.wofs_area_percent+df.wet_percent+df.bare_soil_percent})
            df1 = df1.set_index(df.index)

            line_test = df1.plot(ax=ax2, legend=False,
                                colors='black', **pandasplot_kwargs)
            ...
            def update_figure(frame_i):
                ...
                for i, line in enumerate(line_test.lines):
                    ...
                    y = df1[df1.index <= datetime(
                        year=year, month=month, day=day, hour=23, minute=59)].iloc[:, i]
                    x = df1[df1.index <= datetime(
                        year=year, month=month, day=day, hour=23, minute=59)].index
            ...
            ani = animation.FuncAnimation(
                fig=fig, func=update_figure, frames=timesteps, interval=interval, blit=True)
...
def _add_colourbar(ax, im, vmin, vmax, cmap='Greys', tick_fontsize=15, tick_colour='black'):
    ...
    plt.gcf().colorbar(im, cax=axins2, orientation='horizontal',
